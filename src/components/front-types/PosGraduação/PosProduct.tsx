...
const PosProduct = ({ singleProduct }: { singleProduct: Products }) => {
	const breadcrumbs = [
	...
	const coordinatorOverview = singleProduct?.Teachers?.find(
		(item) =>
			item.Name.toLowerCase() === singleProduct?.CoordinatorName.toLowerCase(),
	)?.About;
		...
		<>
			<Container
				...
				sx={{
					...
					backgroundImage: "linear-gradient(90deg, #1d3557 0%, #ca7b1c 100%)",
				}}
			>
				<Container>
					<Grid container p={5}>
			...
			{/* <SurprisePosCoupon
				couponText={
					<>
						<p>
							Parabéns! Até dia 30/01, estamos numa campanha promocional e você
							ganhou um presente especial!
						</p>
						<p>
							20% DE DESCONTO utilizando o CUPOM{" "}
							<span style={{ fontWeight: 700 }}> POS2025 </span>
						</p>
					</>
				}
				couponName={"POS2025"}
			...
			<Container>
				<Attributes attributes={ecommerceAttributes} color="#CA7B1C" />
			</Container>
			<Container sx={{ mt: 5 }}>
				<H2 fontSize={28} color={"#C4791E"} pt={4}>
					SOBRE O CURSO
				</H2>
				<Box
			...
			<Container>
				...
				<Grid container my={4}>
					<Grid item xs={12} md={4} textAlign={"center"}>
						<AutoStoriesIcon sx={{ fontSize: 50, color: "#3D526F" }} />
						<H5 sx={{ color: "#CC7B1C", fontWeight: "bolder" }}>BIBLIOTECA</H5>
						<p>
					...
					<Grid item xs={12} md={4} textAlign={"center"}>
						<RouteIcon sx={{ fontSize: 50, color: "#3D526F" }} />
						<H5 sx={{ color: "#CC7B1C", fontWeight: "bolder" }}>
					...
					<Grid item xs={12} md={4} textAlign={"center"}>
						<ConstructionIcon sx={{ fontSize: 50, color: "#3D526F" }} />
						<H5 sx={{ color: "#CC7B1C", fontWeight: "bolder" }}>
